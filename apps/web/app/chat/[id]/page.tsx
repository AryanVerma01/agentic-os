...
export default function SingleChatPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    const { messages, sendMessage, isStreaming, terminalOutput } = useAgentStream(sessionId);
    ...
    return (
        <div className="flex h-full w-full bg-gray-100 overflow-hidden">
            ...
            {/* LEFT PANEL: The Chat */}
            <div className="w-1/2 h-full flex flex-col bg-white border-r border-gray-200 p-4 relative">
                {/* Header */}
                ...
                {/* Input Form */}
                <form onSubmit={handleSubmit} className="flex gap-2 items-end bg-gray-50 p-2 rounded-xl shadow-sm border border-gray-200">
            ...
            {/* RIGHT PANEL: The Live Workspace */}
            <div className="w-1/2 h-full flex flex-col bg-gray-900 text-green-400 font-mono text-sm p-4">
                <div className="flex justify-between items-center mb-4 pb-2 border-b border-gray-700">
                    <h2 className="text-gray-100 font-bold tracking-widest">AGENT WORKSPACE (Docker)</h2>
                    <span className="bg-green-900 text-green-200 px-2 py-1 rounded text-xs animate-pulse">
                        SANDBOX ACTIVE
                    </span>
                </div>
                ...
                <div className="flex-1 overflow-y-auto whitespace-pre-wrap">
                    {!terminalOutput ? (
                        <span className="text-gray-600">Waiting for agent to execute code...</span>
                    ) : (
                        terminalOutput
                    )}
                </div>
